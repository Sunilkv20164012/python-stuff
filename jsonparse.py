...
def main():
    ...
    try:
        ...
        if ( len(sys.argv)<2 or (sys.argv[1] is None) or (not sys.argv[1])):
            print("file name argument missing ", sys.argv[1])
            raise TypeError("foo() expected file name arguments, got %s", sys.argv[1])

        file_name = sys.argv[1]
        print("file name received " , file_name)
        
        res = JSONParse(file_name)
